...
const ProfileCard = ({
    ...
    const [swipeDisabledAlertVisible, setSwipeDisabledAlertVisible] = useState(false);

    const route = useRouter();
    const userId = useAuthStore((state) => state.userId);
    const { mutate: sendConnection } = useSendConnection();

    // Define the condition for allowing swipes
    const canSwipe = useMemo(() => {
        return Boolean(profile.job_title || profile.city === "New York");
    }, [profile]);
        ...
        sendConnection(
            ...
            },
            {
                onSuccess: (res) => {
                    ...
                    // Trigger swipe complete after request is sent
                    setTimeout(() => {
                        onSwipeComplete(profile.user_id, "right");
                    }, 1000); // Delay to show toast before moving to next card
                },
                onError: (error) => {
    ...
    }, [userId, profile, sendConnection, setError, onSwipeComplete]);
    ...
    const showSwipeDisabledAlert = useCallback(() => {
        setSwipeDisabledAlertVisible(true);
    }, []);
    ...
    const isGestureEnabled = !isExpanded && canSwipe;
    ...
    const panGesture = Gesture.Pan()
        ...
        .onEnd(() => {
            ...
            if (!canSwipe) {
                runOnJS(showSwipeDisabledAlert)();
        ...
        <>
                                ...
                                >
                                    <View style={styles.detailContent}>
                                        ...
                                        <Text
                                            style={[
                                                styles.location,
                                                { fontSize: 14, color: "#000", fontFamily: FONT.BOLD },
                                            ]}
                                        >
                                            {profile.city || ""}
                                        </Text>
                                        ...
                                        <View style={styles.section}>
                                            ...
                                            <TouchableOpacity
                                                onPress={() => completeSwipe(profile.user_id, "left")}
                                                activeOpacity={0.7}
                                                style={styles.actionButton}
                                            >
                                                <Text style={styles.buttonText}>Reject Profile</Text>
                                            </TouchableOpacity>
                                        </View>
                                    </View>
                                </LinearGradient>
            ...
            <BlockUserModal
                ...
                onSubmit={() => {
                    ...
                    completeSwipe(profile.user_id, "left");
                }}
            ...
            />
            <AlertModal
                visible={swipeDisabledAlertVisible}
                onClose={() => setSwipeDisabledAlertVisible(false)}
                imageSource={require("../../../../assets/icons/cross.png")}
                label="Swiping disabled: Profile does not meet criteria"
                buttonText="OK"
                viewButton
                onButtonPress={() => setSwipeDisabledAlertVisible(false)}
                positionTop
            />
            <AlertModal
...
const Connect = () => {
    ...
    const handleSwipeComplete = useCallback(
        (user_id, direction) => {
            console.log(`Swipe complete: ${user_id}, Direction: ${direction}`);
            if (direction === "left") {
                ...
                setSwipedIds((prev) => {
                    ...
                    if (updated.length >= recommendations.length && !showLimitModal) {
                ...
                undoTimeoutRef.current = setTimeout(() => {
                    console.log("Undo timeout expired");
                    setShowUndo(false);
        ...
        [showLimitModal, recommendations.length]
    );

    const handleUndoComplete = useCallback(() => {
        console.log("Undo triggered for profile:", lastRejectedProfileId);
        if (undoTimeoutRef.current) {
            clearTimeout(undoTimeoutRef.current);
    ...
    const renderItem = useCallback(
        ...
        [rightSwipeCount, handleSwipeComplete, expandedProfileId, handleToggleDetails, setError]
    );
    ...
    const visibleProfileData = useMemo(() => {
        ...
        // Show only the first unswiped profile
        const nextProfile = recommendations.find((item) => !swipedIds.includes(item.user_id));
        return nextProfile ? [nextProfile] : [];
